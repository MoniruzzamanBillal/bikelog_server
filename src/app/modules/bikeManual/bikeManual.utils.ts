...
export const chunkManualText = (rawText: string): string[] => {
  ...
  for (let start = 0; start < words.length; start += CHUNK_STEP) {
    ...
    if (chunks.length >= MAX_CHUNKS_PER_MANUAL) break;
    if (start + CHUNK_WORD_SIZE >= words.length) break;
  }
...
export const scoreAndRankChunks = (
  chunks: TBikeManualChunk[],
  question: string,
  topK: number,
): TBikeManualChunk[] => {
  ...
  const chunkTokenLists = chunks.map((chunk) => tokenize(chunk.chunkText));
  const chunkCount = chunks.length;

  const docFrequencyByToken = new Map<string, number>();
  ...
  const scoredChunks = chunks.map((chunk, i) => {
    ...
    for (const qToken of questionTokens) {
      ...
      const df = docFrequencyByToken.get(qToken) ?? 0;
